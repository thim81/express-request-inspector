...
let broadcastUrl = process.env.INSPECTOR_BROADCAST_URL || 'http://localhost:4004/sse';

// Optionally, update the broadcast URL if needed.
function setBroadcastUrl(url) {
    broadcastUrl = url;
    console.log(`Inspector broadcast URL set to ${broadcastUrl}`);
}
...
function broadcast(data) {
    ...
    req.on('error', err => {
        console.error('Inspector broadcast error:', err);
    });
 ...
 */
function print(data) {
    console.log(`${data.response.status} - ${data.request.method} ${data.endpoint} (${data.latency}ms) - ${data.timestamp}`);
    return data;
}
